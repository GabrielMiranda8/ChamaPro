...
public class UICaracteristica {
    ...
    public void CadastrarCaracteristica(int largura) {
        ...
        Profissional p = null;
        if (idProf > 0) {
            p = Sistema.getInstance().BuscarProfissional(idProf);
            if (p == null) {
    ...
    public void ListarCaracteristicas(int largura) {
        ...
        for (Caracteristica c : lista) {
            if (c != null) {
                ...
                System.out.printf("%-" + largura + "s", c.getProfissionais().isEmpty() ? "nenhum" : c.getProfissionais().size() + " prof(s)");
                System.out.println();
            }
        }
    }

    public void AlterarCaracteristica(int largura) {
        ...
        String nome = sc.nextLine();
        if (nome != null && !nome.trim().isEmpty()) c.setNome(nome);

        System.out.print("Nova descrição (vazio para manter): ");
        String descricao = sc.nextLine();
        if (descricao != null && !descricao.trim().isEmpty()) c.setDescricao(descricao);

        boolean ok = controle.atualizarCaracteristica(c);
        if (ok) System.out.println("Característica atualizada.");
        else System.out.println("Falha ao atualizar característica.");
    }
    ...
    public void AtribuirCaracteristicaAProfissional(int largura) {
        ...
        Profissional p = Sistema.getInstance().BuscarProfissional(idProf);
        if (p == null) {
