...
public class ProfissionalServicoService {
    ...
    public List<ProfissionalServicoResponseDTO> listarPorProfissional(String idProfissional) {
        ...
        return psRepository.findByProfissional_Id(idProfissional)
                .stream()
                .map(ProfissionalServicoResponseDTO::new)
                .toList();
    }

    @Transactional
    public void excluirPorProfissionalServico(String idProfissional, String idServico) {
        ProfissionalServico ps = psRepository.findByProfissional_IdAndServico_Id(idProfissional, idServico)
                .orElseThrow(() -> new ResourceNotFoundException("Serviço não encontrado para esse profissional."));
